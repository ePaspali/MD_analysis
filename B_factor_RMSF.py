...
# -*- coding: utf-8 -*-
"""
Created on Tue Feb 18 17:49:09 2025
...
# load RMSF data (inactive state)
rmsf_file_path = '/path/to/file/rmsf_inactive1_50ns.txt'  # RMSF data of inactive state
rmsf_data = np.loadtxt(rmsf_file_path, usecols=(0, 1))

# load PDB file and extract B-factor values
pdb_file_path = '/path/to/file/7br3_clean.pdb'  # PDB file for B-factor values
    ...
    with open(pdb_file, 'r') as file:
        for line in file:
            if line.startswith('ATOM'):
                residue_id = int(line[22:26].strip())  # residue ID is extracted
                b_factor = float(line[60:66].strip())  # B-factor is extracted
...
# normalise both RMSF and B-factor values
...
plt.ylabel('Normalized Value', fontsize=22, fontweight='bold')
plt.xlim(18, 328)
...
# mark transmembrane regions with labels
...
for midpoint, label in zip(midpoints, ['TM1', 'TM2', 'TM3', 'TM4', 'TM5', 'TM6', 'TM7']):
    plt.text(midpoint, max(normalized_b_factors), label, ha='center', va='center', color='black', fontsize=18)

# highlight transmembrane regions
